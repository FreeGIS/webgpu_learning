...
export const description = '渲染一个边界平滑的圆';
...
export const wgslShaders = {
  ...
  [[stage(vertex)]]
  fn main() -> void {
    vec2<f32> zeroToOne = a_position/uniforms.bounds;
    vec2<f32> oneToTwe = zeroToOne*2.0;
    vec2<f32> clipSpace = oneToTwe-1.0;
...
`,
  ...
  [[stage(fragment)]]
  fn main() -> void {
    float dis = distance(coord.xy,uniforms.circle.xy);
    if(dis<uniforms.circle.z-3.0){
      outColor=vec4<f32>(1.0,0.0,0.0,1.0);
    }
    else if(dis<=uniforms.circle.z){
      float step=smoothstep(uniforms.circle.z-3.0,uniforms.circle.z,dis);
      outColor=mix(vec4<f32>(1.0,0.0,0.0,1.0), vec4<f32>(0.0,0.0,0.0,1.0), step);
    }
    else{
