...
async function loadExample(hashName: string) {
    ...
    //要注意的是，默认交换链的图像尺寸是来自于 <canvas> 的 width 和 height 属性，而不是 clientWidth 和 clientHeight
    canvas.width = 600;
    canvas.height = 600;
    ...
    linkNode.href = `https://github.com/FreeGIS/webgpu_learning/tree/master/src/examples/${name}.ts`;
